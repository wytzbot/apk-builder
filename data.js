...
export async function listRunArtifacts(token, username, runId) {
  ...
  if (!res.ok) {
    const err = new Error(`Failed to list artifacts for run ${runId} (${res.status})`);
    if (res.status === 403 || res.status === 429) {
      const resetHeader = res.headers.get("x-ratelimit-reset");
      const retryAfter = res.headers.get("retry-after");
      if (retryAfter) err.retryAfterMs = Number(retryAfter) * 1000;
      else if (resetHeader) err.retryAfterMs = Math.max(1000, Number(resetHeader) * 1000 - Date.now());
    }
    throw err;
  }
  const data = await res.json();
...
/**
 * GitHub's artifact index can lag a few seconds behind a run flipping to
 * "completed" (the run object updates before the artifacts API has caught
 * up). Calling listRunArtifacts exactly once right after completion is what
 * causes the "download button appears but there's nothing to download"
 * symptom. This retries with backoff until artifacts actually show up, with
 * an onProgress callback so the UI can say what's happening, and a bounded
 * number of attempts so we never spin forever.
 */
export async function waitForArtifacts(token, username, runId, { maxAttempts = 6, delayMs = 2000, onProgress } = {}) {
  let lastError = null;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const artifacts = await listRunArtifacts(token, username, runId);
    ...
    if (attempt < maxAttempts) {
      await new Promise(r => setTimeout(r, delayMs * attempt)); // gentle backoff
    }
  }
  if (lastError) throw lastError;
  return []; // ran out of attempts — genuinely no artifacts (e.g. workflow didn't upload one)
}

/**
 * Wraps a GitHub API call so a single transient failure (network blip,
 * secondary rate limit, momentary 5xx) doesn't tear down a whole polling
 * loop. Retries a few times with backoff before giving up for real.
 * Honors the retry-after hint attached above when GitHub sends one.
 */
export async function withRetry(fn, { retries = 3, baseDelayMs = 1500 } = {}) {
  ...
  while (true) {
    try {
      ...
      const wait = err && err.retryAfterMs ? err.retryAfterMs : baseDelayMs * attempt;
