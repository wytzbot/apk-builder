...
// How long we'll keep quietly polling before treating this as "stuck" and
// showing the user an explicit option instead of spinning forever.
const POLL_INTERVAL_MS = 6000;
const STALE_AFTER_MS = 8 * 60 * 1000;   // show a "taking longer than usual" hint
const TIMEOUT_AFTER_MS = 20 * 60 * 1000; // stop auto-polling, ask the user what to do
...
async function pollRunUntilDone(token, username, runId, setStage) {
  ...
  while (true) {
    let run;
    try {
      // A single dropped request or momentary GitHub hiccup used to bubble
      // straight up and kill the whole loop, which is what made a live build
      // look like a permanent freeze/failure. withRetry absorbs that here;
      // we only give up on the loop after several retries in a row fail.
      run = await withRetry(() => getRun(token, username, runId), { retries: 3 });
      consecutiveFailures = 0;
    } catch (err) {
      consecutiveFailures++;
      if (consecutiveFailures >= 5) {
        throw new Error("Lost contact with GitHub Actions while watching this build. Your build may still be running — check it on GitHub, or retry to resume watching.");
      }
    ...
    if (elapsed > TIMEOUT_AFTER_MS) {
      // Never leave the user staring at a spinner indefinitely — hand
      // control back with a clear choice instead of guessing.
      showBuildFailure("This is taking much longer than usual. The build may still be running on GitHub — open it there, or retry to keep watching.");
      return;
    } else if (elapsed > STALE_AFTER_MS && !staleNoticeShown) {
      staleNoticeShown = true;
      const subtitle = document.getElementById("processingSubtitle");
      if (subtitle) subtitle.textContent = "Still going — larger sites/icons can take a while. Hang tight…";
    }
...
function showBuildFailure(message) {
  ...
  const failMsgEl = document.getElementById("processingFailedMessage");
  if (failMsgEl) failMsgEl.textContent = message;
  const subtitle = document.getElementById("processingSubtitle");
...
async function completeBuild(token, username, runId) {
  // The run object can flip to "completed" a beat before GitHub's artifact
  // index catches up. Checking once (the old behavior) is exactly what
  // produced a download button with nothing behind it. We wait for the
  // real thing instead, and tell the user what's happening while we do.
  const subtitle = document.getElementById("processingSubtitle");
  let artifacts = [];
  let artifactsFailed = false;
  try {
    artifacts = await waitForArtifacts(token, username, runId, {
      maxAttempts: 6,
      delayMs: 2000,
      onProgress: () => {
        if (subtitle) subtitle.textContent = "Build finished — finalizing your APK for download…";
      }
    });
  } catch (err) {
  ...
  state.currentBuild.artifactsPending = artifacts.length === 0;
  ...
  // Swap the download button for a "Check Again" state instead of leaving a
  // button that will just fail silently when tapped.
  const downloadBtn = document.getElementById("downloadApkBtn");
  const pendingNote = document.getElementById("dlArtifactPending");
  if (artifacts.length === 0) {
    if (downloadBtn) {
      downloadBtn.textContent = artifactsFailed ? "Check Again" : "Check Again (still finalizing)";
      downloadBtn.onclick = () => recheckArtifacts();
    }
    if (pendingNote) pendingNote.classList.remove("hidden");
  } else {
    if (downloadBtn) {
      downloadBtn.textContent = "Download APK";
      downloadBtn.onclick = () => downloadApk();
    }
  ...
  initHistory();
  showScreen("download");
  showToast(artifacts.length ? "APK successfully generated!" : "Build succeeded — still preparing the download.", artifacts.length ? "success" : "info");
}

/** Manual re-check for the rare case artifacts still aren't indexed after
 *  completeBuild's own retries — lets the user nudge it without restarting
 *  the whole build. */
async function recheckArtifacts() {
  const build = state.currentBuild;
  if (!build || !build.runId) return;
  triggerHaptic();
  const token = localStorage.getItem("wyte_github_token");
  showToast("Checking for your APK…", "info");
  try {
    const artifacts = await listRunArtifacts(token, state.user.login, build.runId);
    if (artifacts.length > 0) {
      build.artifacts = artifacts.map(a => ({ id: a.id, name: a.name, sizeBytes: a.size_in_bytes }));
      build.artifactsPending = false;
      localStorage.setItem("wyte_builds", JSON.stringify(state.builds));
      document.getElementById("dlSize").textContent = `${(artifacts[0].size_in_bytes / (1024 * 1024)).toFixed(1)} MB (zipped)`;
      const downloadBtn = document.getElementById("downloadApkBtn");
      const pendingNote = document.getElementById("dlArtifactPending");
      if (downloadBtn) {
        downloadBtn.textContent = "Download APK";
      ...
      if (pendingNote) pendingNote.classList.add("hidden");
      showToast("Found it!", "success");
    } else {
      showToast("Not indexed yet — give it a few more seconds and try again.", "warning");
    }
  } catch (err) {
    showToast("Still couldn't reach GitHub — try opening the run directly.", "warning");
  }
}
...
function retryBuild() {
  ...
  // If we already have a GitHub run for this build, the failure almost
  // certainly happened while *watching* it (a dropped connection, a rate
  // limit, an artifact-index lag) — not in the build itself. Resume
  // watching that same run rather than dispatching a whole new workflow
  // run (which wastes the user's Actions minutes and their time).
  if (state.currentBuild && state.currentBuild.runId) {
    const token = localStorage.getItem("wyte_github_token");
    const username = state.user.login;
    document.getElementById("processingSubtitle").textContent = "Reconnecting to your build…";
    let elapsedSeconds = 0;
    const elapsedEl = document.getElementById("elapsedTime");
    state.buildInterval = setInterval(() => {
      elapsedSeconds++;
    ...
    pollRunUntilDone(token, username, state.currentBuild.runId, (text) => {
      ...
      clearInterval(state.buildInterval);
      showBuildFailure(err.message || "Still couldn't reach the build.");
    });
    return;
  }

  // No run yet — the failure happened before/during dispatch, so a fresh
  // attempt really is the only option.
  runRealBuild();
}
